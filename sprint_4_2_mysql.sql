...
JOIN 
( 
		SELECT 		CAST(SUBSTRING_INDEX(SUBSTRING_INDEX(transactions.product_ids, ',', product_id), ',', -1) AS UNSIGNED) AS product,
		...
		GROUP BY	product ORDER BY 	product) AS tabla
ON tabla.product = products.id;